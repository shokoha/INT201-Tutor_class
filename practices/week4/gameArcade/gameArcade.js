class GameArcade {
    ...
    addGame(name) {
        if (this.games.some(game => game.gameName === name)) {
            return -1
        }
        this.games.push({ gameId: GameArcade.autoId++, gameName: name })
    ...
    removeGame(id) {
        if(this.findGameId(id) === -1 ) return -1
        this.games.splice(this.findGameId(id),1)
        return id
    }
    getGameById(id) {
